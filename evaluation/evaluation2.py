...
개선된 평가 방식으로 모든 이미지에서 얻은 confusion matrix를 통해 mIoU 계산 (Pstage3 segementation 대회에서 적용 X)
'''
import numpy as np
import pandas as pd
...
def evaluation(gt_path, pred_path):
    ...
    assert len(pred_df) == 837, 'Inconsistent number of test sets'
    ...
    for index, (file_name, pred) in enumerate(zip(pred_file_names, predictions)):
        ...
        if file_name in json_file_name:
            ...
            if image_infos['file_name'] == file_name:
                ...
                # mIoU 2 (new) 모든 image에 대해 confusion matrix sum
                for lt, lp in zip(pred, masks):
    ...
    return label_accuracy_score(hist) # np.mean(mIoU_list) #
